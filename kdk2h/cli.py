...
def _report_progress(label: str, current: int, total: int, last_percent: int) -> int:
    if total <= 0:
        return last_percent
    percent = min(100, int((current * 100) / total))
    if percent == 100 or percent >= last_percent + 5:
        print(f"{label}: {percent}%", file=sys.stderr)
        return percent
    return last_percent
...
def _print_reverse_dependencies(cu_prefix: str, root_die: Any) -> None:
    ...
    print("/* Reverse dependency order (base types first) */")
    print("/* C header-style declarations generated from DWARF */")
    emitted_count = 0
...
def _build_macho_dwarf_info(dwarf_path: Path, header: Any) -> DWARFInfo | None:
    ...
    total_bytes = sum(size for _, _, size in section_meta)
    read_bytes = 0
    last_percent = 0
    ...
    with dwarf_path.open("rb") as stream:
        for section_name, section_offset, section_size in section_meta:
            ...
            read_bytes += section_size
            last_percent = _report_progress("Reading file", read_bytes, total_bytes, last_percent)
...
def main() -> int:
    ...
    source_group.add_argument(
        "--kdk",
        type=str,
        help="KDK tag in the form <platform>@<version>.",
    )
    parser.add_argument(
        "type_name",
        nargs="?",
    ...
    try:
        dwarf_file = _resolve_input_path(kdk_file=args.kdk_file, kdk_tag=args.kdk)
        ...
        if args.type_name:
            def print_named_type(dwarf_infos: list[tuple[str, DWARFInfo]]) -> bool:
                ...
                if args.with_dependency_tree:
                    ...
                    print()
                _print_reverse_dependencies(cu_prefix, die)
                return True
